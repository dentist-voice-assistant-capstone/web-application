...
def preprocess_test_sentence_from_start(test_sentences, auto_tokenizer):
    test_sentences = auto_tokenizer.tokenize(test_sentences)
    test_sentences = " ".join(test_sentences)
    return test_sentences


def postprocess_predictions_BI(predictions, mode="normal"):
    result_list = []
...
def initial_session_and_argument(onnx_model_path, onnx_model_argument_path, tokenizer):
...
def prediction_with_onnx(initial_list, test_sentence):
    [
        session,
        label_list,
        max_seq_length,
